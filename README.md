# Tucil 1 : Penyelesaian Permainan Queens Linkedin
#### Muhammad Naufal Romi Annafi - 13524058 

---

## Overview
This project is an assignment for the **IF2211 Strategi Algoritma** course. The purpose of this assignment is to deepen our understanding of brute-force algorithms in problem-solving by implementing and analyzing their effectiveness in various scenarios. Through this project, we explore the strengths and limitations of brute-force approaches, gaining insights into their practical applications and computational complexity.

---

## About
**Queens** is a logic game available on the professional networking site LinkedIn. The objective of the game is to place queens on a colored square board so that there is only one queen in each row, column, and color region. Additionally, a queen cannot be placed adjacent to another queen, including diagonally. This repository features a solver for the Queens logic game, implemented using a brute-force search algorithm.

## Installation & Setup
1. Make sure you already have Python installed
2. Clone the repository
    ```sh
   git clone https://github.com/naufalromi/Tucil1_13524058.git
   ```
3. Open the terminal 
4. Run this command
   ```
   python3 src/solver.py
      ```

## Other
| No | Points | Yes | No |
| --- | --- | --- | --- |
| 1 | Program berhasil dikompilasi tanpa kesalahan  | ✔️ |  |
| 2 | Program berhasil dijalankan | ✔️ | |
| 3 | Solusi yang diberikan program benar dan mematuhi aturan permainan | ✔️ | |
| 4 | Program dapat membaca masukan berkas .txt serta menyimpan solusi dalam berkas .txt | ✔️ | |
| 5 | Program memiliki Graphical User Interface (GUI) |  | ✔️ |
| 6 | Program dapat menyimpan solusi dalam bentuk file gambar |  | ✔️ |

<br/>
<br/>
<br/>

<div align="center">
Muhammad Naufal Romi Annafi • © 2026 • 13524058
</div>